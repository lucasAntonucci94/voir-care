...
const commentsRef = collection(db, 'groupComments');

export function useGroupComments(groupId, postId) {
  const comments = ref([]);
  ...
  async function saveComment({ user, message }) {
    ...
    try {
      ...
      const post = await getPostById(groupId, postId);
  ...
  async function deleteComment(idDoc) {
    try {
      const docRef = doc(db, 'groupComments', idDoc);
  ...
  function subscribeToComments() {
    ...
    if (!groupId) {
      console.warn('No se proporcionó un groupId para suscribirse a comentarios');
      return;
    }
