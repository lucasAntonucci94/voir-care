<template>
  <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-101 transition-opacity duration-300">
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg mx-4 shadow-2xl transform transition-all duration-300 scale-100 relative max-h-[90vh] overflow-y-auto">
      <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100 mb-6 tracking-tight">Nueva publicación en el grupo</h2>
      <form @submit.prevent="createPost" class="space-y-6">
        <!-- Título -->
        <div>
          <label for="group-post-title" class="sr-only">Título</label>
          <input 
            id="group-post-title"
        ...
        <!-- Descripción -->
        <div>
          <label for="group-post-description" class="sr-only">Descripción</label>
          <textarea 
        ...
        <!-- Input de archivo -->
        <div class="relative">
          <label for="group-post-media" class="sr-only">Imagen o video</label>
          <input 
            id="group-post-media"
            type="file" 
            accept="image/*,video/*" 
...
const showModal = ref(true); // Modal siempre visible al montar el componente
...
// Cerrar modal si groupId cambia a null
...
.custom-checkbox:checked {
  background-color: #02bcae; /* bg-primary */
  border-color: #02bcae;
}

.custom-checkbox:checked:hover {
  background-color: #019a8e; /* hover:bg-primary-md */
  border-color: #019a8e;
}
...
.custom-checkbox:focus {
  box-shadow: 0 0 0 3px rgba(52, 152, 219, 0.5); /* focus:ring-primary */
}
</style>