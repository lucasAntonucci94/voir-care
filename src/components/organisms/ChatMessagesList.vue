<template>
    <div v-if="privateChatsStore.selectedChatId" class="w-full md:w-2/3 bg-gray-100 dark:bg-gray-800 rounded-xl shadow-xl p-5 flex flex-col h-full border border-gray-200 dark:border-gray-800" style="min-height: calc(100vh - 120px); max-height: 80vh; position: relative;">
        ...
        >
          <div class="relative">
            ...
            <span 
              ...
              >{{ msg?.user.email }} - {{ formatDate(msg.created_at) }}</span>
            <button
      ...
      <ChatMessageInput :selectedChatId="privateChatsStore?.selectedChatId" class="mt-auto p-3 bg-gray-100 border-t border-gray-200" />
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-101 flex items-center justify-center">
            ...
            <div class="flex justify-end space-x-4">
                <button @click="closeDeleteModal" class="px-4 py-2 text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-200 dark:bg-gray-600 dark:hover:bg-gray-500 rounded-xl transition-colors duration-200">Cancelar</button>
                <button @click="deleteMessage(messageToDelete)" class="px-4 py-2 bg-primary dark:bg-secondary text-white rounded-xl hover:bg-primary-md dark:hover:bg-secondary-md transition-colors duration-200">Eliminar</button>
            </div>
            </div>
        </div>
    </div>
    <div v-else class="w-full md:w-2/3 bg-white dark:bg-gray-800 dark:text-gray-300 rounded-xl shadow-xl p-5 overflow-y-auto text-center text-gray-700 border border-gray-200 dark:border-gray-700" style="max-height: 80vh;">
