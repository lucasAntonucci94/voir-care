...
<template>
  ...
  <EventPostEditModal :visible="showModalEdit" :post="post" @close="closeEditModal" @update-post="handlePostUpdate" />
...
import EventPostEditModal from './EventPostEditModal.vue';
...
const isSaved = computed(() => {
  return eventPostsStore.savedPostIds.includes(props.post.idDoc);
});

// Toggle save/unsave post
async function toggleSavePost() {
  if (!user.value?.uid) return;
  await eventPostsStore.toggleSavePost(user.value.uid, props.post.idDoc, props.post.event.id);
  props.post.showMenu = false;
...
async function handleReport() {
  ...
  const response = await saveReport({
      ...
      metadata: { title: props.post.title, eventId: props.post.event.id },
    });
...
async function handleHidePost() {
  ...
  if (response) {
    ...
    document.body.style.overflow = '';
    user.value?.hiddenEventPosts?.push({ id: user.value.uid, postId: props.post.id });
