<template>
    <div>
      <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4 sr-only">Publicaciones Guardadas</h2>
      <div v-if="isLoading" class="flex justify-center items-center py-6">
        <img src="../../assets/loaders/pawOrange.gif" alt="Loading...">
        <p class="text-primary dark:text-secondary ml-2 animate-pulse">Cargando publicaciones guardadas...</p>
      </div>
      <div v-else-if="filteredHiddenPosts?.length > 0" class="space-y-6 mx-auto max-w-lg">
        <HiddenPostCard
          v-for="post in filteredHiddenPosts"
          :key="post.idDoc"
          :post="post"
        />
      </div>
      <div v-else class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-md">
        <p class="text-gray-500 dark:text-gray-400">No tienes publicaciones ocultas.</p>
        <button
          class="mt-4 px-4 py-2 bg-primary hover:bg-primary-md dark:bg-secondary dark:hover:bg-secondary-md text-white rounded-lg text-sm transition-colors"
          @click="navigateToFeed"
        >
          Explorar publicaciones
        </button>
      </div>
    </div>
  </template>
  
  <script setup>
  import { ref, watch, computed } from 'vue'
  import { useRouter } from 'vue-router'
  import { useAuth } from '../../api/auth/useAuth'
  import { usePostsStore } from '../../stores/posts'
  import PostCard from '../organisms/PostCard.vue'
  import HiddenPostCard from '../molecules/HiddenPostCard.vue'
  
  const router = useRouter()
  const { user } = useAuth()
  const postsStore = usePostsStore()
  const isLoading = ref(true);
  
  const userHiddenPosts = computed(() => user.value?.hiddenPosts || [])

 watch(
  () => userHiddenPosts.value,
  async (newHiddenPosts) => {
    if (newHiddenPosts.length > 0) {
      isLoading.value = true
      const postIds = newHiddenPosts.map(post => post.postId)
      await postsStore.fetchHiddenPosts(postIds)
      isLoading.value = false
    } else {
      postsStore.hiddenPosts = []
      isLoading.value = false
    }
  },
  { immediate: true }
)

const filteredHiddenPosts = computed(() => {
    return postsStore.hiddenPosts
  })
  
  function navigateToFeed() {
    router.push('/feed')
  }
  </script>
  
  <style scoped>
  /* Add any necessary scoped styles if needed */
  </style>