<template>
  <div class="min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-800 dark:text-white">Administración de Categorías de Blogs</h1>
      <button
        @click="openCreateModal"
        class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark flex items-center gap-2"
        aria-label="Crear nueva categoría de blog"
      >
    ...
    <div v-else>
      ...
      <div class="mb-6 flex flex-col sm:flex-row gap-4">
        <input
          ...
          aria-label="Buscar categorías de blogs"
        />
      </div>
    ...
    >
      <div class="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-md">
        ...
        <form @submit.prevent="saveCategory">
          ...
          <div class="mb-4">
            <label class="block text-gray-700 dark:text-gray-300 mb-1" for="slug">Slug (opcional)</label>
            <input
              id="slug"
              v-model="editCategory.slug"
              type="text"
              placeholder="Ej: adopcion"
              class="w-full p-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
            />
          </div>
...
const { blogCategories, loadBlogCategories, saveBlogCategorie, updateBlogCategorie, deleteBlogCategorie } = useCategories();
...
const editCategory = ref({ name: '', slug: '', idDoc: null });
...
onMounted(async () => {
  ...
  try {
    ...
    error.value = 'Error al cargar categorías de blogs. Intenta de nuevo.';
  } finally {
...
const filteredCategories = computed(() => {
  let filtered = blogCategories.value;
...
const openCreateModal = () => {
  ...
  editCategory.value = { name: '', slug: '', idDoc: null };
...
const openEditModal = (category) => {
  ...
  editCategory.value = { ...category, slug: category.slug || '' };
  showModal.value = true;
};
...
const saveCategory = async () => {
  try {
    if (!editCategory.value.name.trim()) {
      snackbarStore.show('El nombre es obligatorio', 'error');
      return;
    }
    const categoryData = {
      name: editCategory.value.name,
      slug: editCategory.value.slug.trim() || undefined,
    };
    if (isEditing.value) {
      await updateBlogCategorie(editCategory.value.idDoc, categoryData);
      snackbarStore.show('Categoría actualizada exitosamente', 'success');
    } else {
      await saveBlogCategorie(categoryData);
...
const deleteCategory = async (docId) => {
  ...
  try {
    await deleteBlogCategorie(docId);
